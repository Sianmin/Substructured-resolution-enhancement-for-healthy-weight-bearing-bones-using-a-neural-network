...
def WriteTecplotFiles(model, subject, ratio=1, SED=True, VonStress=True, Displacement=False, onlyTra = False):
    ...
    if VonStress:
        ...
        for load_case in range(1, case_n+1):
            ...
            else: fn = "FEA/%s/Plot/ROI_VonStress_s%d_c%d_PLOT.PLT" % (model, subject, load_case)
    ...
    if SED:
        ...
        for load_case in range(1, 4):
            ...
            else: fn = "FEA/%s/Plot/ROI_SED_s%d_c%d_PLOT.PLT" % (model, subject, load_case)
            with open(fn, 'w') as fid:
...
def WriteTecplotFiles_ROI(model, subject, ratio=1, SED=True, VonStress=True, Displacement=False, onlyTra = False):
    ry, rx = 325, 1250
    side_length = 300
    ...
    if VonStress:
        ...
        Von_Flag = Von_ref > 0
        Von_Flag = Von_Flag[:, :, 0]
        elem_n = np.sum(Von_Flag[ry:ry+side_length, rx:rx+side_length])
        ...
        for load_case in range(1, case_n+1):
            ...
            with open(fn, 'w') as fid:
                ...
                fid.write("VARIABLES = \"X (mm)\", \"Y (mm)\", \"Von Mises Stress (MPa)\"\n")
                fid.write("ZONE N=%d, E=%d, F=FEPOINT, ET=QUADRILATERAL\n" % (elem_n*4, elem_n))
                for i in range(ry, ry+side_length):
                    for j in range(rx, rx+side_length):
                        if Von_Flag[i, j]:
                            for ii in range(2):
                                for jj in range(2):
                                    fid.write("%f, %f, %f\n" % ((j+jj)*resolution, (i+ii)*resolution, Von_ref[i, j, load_case-1]))
